...
- **多种命名模式**：支持“来源+时间戳”、“固定名称”以及“自动增量序号”。
- **自定义自增逻辑**：可配置自增序列类型（数字、小写字母、大写字母）及前缀。
- **状态持久化**：自增序号记录在配置文件中，确保在程序重启后依然能够延续之前的序号。
...
4. **配置命名**：点击 `命名规则...` 可自定义保存的文件名生成逻辑（如时间戳或自增序号），并指定 WAV 文件的保存目录。
