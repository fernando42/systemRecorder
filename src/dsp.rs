        ...
        if self.settings.vocal_enhancement.enable_agc {
            ...
            for ch in 0..channels {
                ...
                if frame_rms > agc.current_rms {
                    // 攻击阶段
                    agc.current_rms = agc.current_rms + attack_factor * (frame_rms - agc.current_rms);
                } else {
                    // 释放阶段
                    agc.current_rms = agc.current_rms + release_factor * (frame_rms - agc.current_rms);
                }
                ...
                agc.current_gain = agc.current_gain + gain_change_factor * (desired_gain - agc.current_gain);
        ...
        if self.settings.vocal_enhancement.enable_eq {
            for i in 0..data.len() {
                ...
                for filter in self.eq_filters[ch].iter_mut() {
                    let y = filter.b0 * sample + filter.b1 * filter.prev_x1 + filter.b2 * filter.prev_x2
                          - filter.a1 * filter.prev_y1 - filter.a2 * filter.prev_y2;
